(() => {
  ...
  notice.innerHTML = '<strong>Connexion sÃ©curisÃ©e.</strong> Vos informations sont protÃ©gÃ©es et chaque client peut uniquement consulter son propre dossier.';
  let demoMode = false;
  const demoButton = document.createElement('button');
  demoButton.type = 'button';
  demoButton.className = 'dashboard-action demo-access';
  demoButton.textContent = 'Voir le tableau de bord dÃ©mo';
  ...
  dashboard.innerHTML = `
    <div class="dashboard-head"><div><div class="eyebrow">ESPACE SÃ‰CURISÃ‰</div><h1 id="welcomeName">Bonjour</h1></div><button class="dashboard-action" id="logoutButton" type="button">DÃ©connexion</button></div>
    ...
    <div class="dashboard-grid">
      <article class="dashboard-card"><span>ABONNEMENT</span><strong id="membershipPlan">Chargementâ€¦</strong><p id="membershipStatus">â€”</p></article>
      <article class="dashboard-card"><span>FACTURES</span><strong id="invoiceCount">0</strong><p>Documents disponibles</p></article>
      <article class="dashboard-card"><span>DOCUMENTS</span><strong id="documentCount">0</strong><p>Fichiers privÃ©s</p></article>
    </div>
    <div class="dashboard-columns">
      <article class="dashboard-panel"><h2>Discussion avec Alfred-EA</h2><p class="chat-note">Vos messages restent dans votre dossier privÃ© et sont visibles par lâ€™Ã©quipe administratrice.</p><div class="message-list chat-list" id="messageList"><p>Aucun message.</p></div><form id="messageForm" class="message-form"><textarea id="messageBody" maxlength="5000" placeholder="Ã‰crire un message privÃ© Ã  Alfred-EA" required></textarea><button class="submit" type="submit">Envoyer le message</button></form></article>
      <article class="dashboard-panel"><h2>Mes factures</h2><div id="invoiceList"><p>Aucune facture disponible.</p></div><button class="dashboard-action subscription-manage" id="manageSubscriptionButton" type="button" aria-expanded="false" aria-controls="subscriptionManagePanel">GÃ©rer mon abonnement</button><div class="subscription-manage-panel" id="subscriptionManagePanel" hidden><p>Modifiez votre niveau ou demandez lâ€™annulation avant le prochain renouvellement.</p><div class="subscription-actions"><a class="subscription-link" href="subscription.html">Voir ou modifier mon niveau</a><button class="subscription-cancel" id="cancelSubscriptionButton" type="button">Demander lâ€™annulation</button></div><p id="subscriptionManageStatus">Lâ€™annulation est confirmÃ©e par lâ€™Ã©quipe Alfred-EA avant de prendre effet.</p></div><h2 class="section-space">Mes documents</h2><div id="documentList"><p>Aucun document disponible.</p></div></article>
    </div>
    <div class="secure-sections">
      <article class="dashboard-panel"><h2>Mes comptes MT5</h2><p class="security-note">Enregistrez jusquâ€™Ã  cinq comptes. Le mot de passe est chiffrÃ©, expire aprÃ¨s 24 heures et chaque consultation exige une nouvelle authentification administrateur.</p><form id="mt5Form" class="secure-form"><div id="mt5Rows"></div><button class="submit" type="submit">Enregistrer et transmettre</button><p class="form-feedback" id="mt5Status" role="status"></p></form></article>
      <article class="dashboard-panel"><h2>Permis de conduire</h2><p class="security-note">TÃ©lÃ©versement privÃ© â€” JPG, PNG, WebP ou PDF, maximum 10 Mo par fichier.</p><form id="licenseForm" class="secure-form"><div class="upload-grid"><div class="upload-box"><label for="licenseFront">Recto du permis</label><input id="licenseFront" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required></div><div class="upload-box"><label for="licenseBack">Verso du permis</label><input id="licenseBack" type="file" accept="image/jpeg,image/png,image/webp,application/pdf" required></div></div><button class="submit" type="submit">Enregistrer mon permis</button><p class="form-feedback" id="licenseStatus" role="status"></p></form></article>
    </div>`;
  ...
  function showDemoDashboard() {
    ...
    document.getElementById('membershipStatus').textContent = 'Actif Â· Renouvellement le 15 septembre 2026';
    document.getElementById('invoiceCount').textContent = '2';
    document.getElementById('documentCount').textContent = '3';
    const messageList = document.getElementById('messageList');
    messageList.replaceChildren();
    addMessageBubble(messageList, 'Bienvenue dans votre Espace client Alfred-EA.', 'Ã‰quipe Alfred-EA Â· Aujourdâ€™hui');
    addMessageBubble(messageList, 'Votre abonnement est actif et votre dossier est Ã  jour.', 'Ã‰quipe Alfred-EA Â· Aujourdâ€™hui');
    const invoiceList = document.getElementById('invoiceList');
    invoiceList.replaceChildren();
    addTextRow(invoiceList, 'AE-2026-002', '60,00 $ CA Â· PayÃ©e');
    addTextRow(invoiceList, 'AE-2026-001', '60,00 $ CA Â· PayÃ©e');
    ...
    addTextRow(documentList, 'Guide de dÃ©marrage', 'Information');
  }
  ...
  document.getElementById('cancelSubscriptionButton').addEventListener('click', () => {
    const subject = encodeURIComponent('Demande dâ€™annulation de mon abonnement Alfred-EA');
    const body = encodeURIComponent('Bonjour,\n\nJe souhaite demander lâ€™annulation de mon abonnement Alfred-EA avant mon prochain renouvellement. Merci de me confirmer la date de fin de mon accÃ¨s.\n\nNom complet :\nAdresse courriel du compte :\n');
  ...
  async function loadDashboard(user) {
    ...
    document.getElementById('membershipPlan').textContent = membershipResult.data?.plan_name || 'Ã€ confirmer';
    ...
    const messageList = document.getElementById('messageList');
    messageList.replaceChildren();
    (messagesResult.data || []).forEach(message => addMessageBubble(messageList, message.body, `${message.sender_id === user.id ? 'Vous' : 'Alfred-EA'} Â· ${new Date(message.created_at).toLocaleString('fr-CA')}`, message.sender_id === user.id));
    if (!messagesResult.data?.length) messageList.innerHTML = '<p>Aucun message.</p>';
    const invoiceList = document.getElementById('invoiceList');
    invoiceList.replaceChildren();
    (invoicesResult.data || []).forEach(invoice => addTextRow(invoiceList, invoice.invoice_number, `${(invoice.amount_cents / 100).toLocaleString('fr-CA', { style: 'currency', currency: invoice.currency })} Â· ${invoice.status}`));
  ...
  signupForm.addEventListener('submit', async event => {
    ...
    status('signupStatus', error ? error.message : 'VÃ©rifiez votre courriel pour confirmer votre compte.', !!error);
  });

  document.querySelector('[data-demo]').addEventListener('click', async event => {
    event.preventDefault();
    const email = document.getElementById('loginEmail').value;
    if (!email) return status('loginStatus', 'Entrez dâ€™abord votre adresse courriel.', true);
    const { error } = await sb.auth.resetPasswordForEmail(email, { redirectTo: location.origin + location.pathname });
    status('loginStatus', error ? error.message : 'Un courriel de rÃ©initialisation a Ã©tÃ© envoyÃ©.', !!error);
  });

  dashboard.addEventListener('submit', async event => {
    if (event.target.id === 'mt5Form') {
      event.preventDefault();
      if (demoMode) return document.getElementById('mt5Status').textContent = 'AperÃ§u : les informations ne sont pas enregistrÃ©es en mode dÃ©mo.';
      ...
      for (let slot = 1; slot <= 5; slot += 1) {
        ...
        if (!broker || !/^[0-9]{3,30}$/.test(account_number)) return document.getElementById('mt5Status').textContent = `VÃ©rifiez le courtier et le numÃ©ro du compte ${slot}.`;
      ...
      if (deleteError || insertError) { document.getElementById('mt5Status').textContent = 'Impossible dâ€™enregistrer pour le moment.'; return; }
      for (let slot = 1; slot <= 5; slot += 1) {
        ...
        passwordInput.value = '';
        if (credentialError) { document.getElementById('mt5Status').textContent = `Comptes enregistrÃ©s, mais le mot de passe du compte ${slot} nâ€™a pas Ã©tÃ© transmis.`; return; }
      }
      document.getElementById('mt5Status').textContent = 'Vos comptes sont enregistrÃ©s. Tout mot de passe fourni expirera dans 24 heures et chaque consultation administrateur sera vÃ©rifiÃ©e et journalisÃ©e.';
      return;
    }
    if (event.target.id === 'licenseForm') {
      event.preventDefault();
      if (demoMode) return document.getElementById('licenseStatus').textContent = 'AperÃ§u : aucun document nâ€™est tÃ©lÃ©versÃ© en mode dÃ©mo.';
      const { data: { user } } = await sb.auth.getUser();
      if (!user) return;
      const files = [['front', document.getElementById('licenseFront').files[0], 'Permis de conduire â€” recto'], ['back', document.getElementById('licenseBack').files[0], 'Permis de conduire â€” verso']];
      if (files.some(([, file]) => !file || file.size > 10485760)) return document.getElementById('licenseStatus').textContent = 'SÃ©lectionnez deux fichiers de 10 Mo maximum.';
      document.getElementById('licenseStatus').textContent = 'TÃ©lÃ©versement en coursâ€¦';
      for (const [side, file, displayName] of files) {
        ...
        if (uploadError) return document.getElementById('licenseStatus').textContent = 'Le tÃ©lÃ©versement a Ã©chouÃ©. RÃ©essayez.';
        ...
        if (recordError) return document.getElementById('licenseStatus').textContent = 'Le document a Ã©tÃ© reÃ§u, mais son dossier doit Ãªtre vÃ©rifiÃ©.';
      }
      event.target.reset();
      document.getElementById('licenseStatus').textContent = 'Votre permis a Ã©tÃ© enregistrÃ© de faÃ§on privÃ©e.';
    ...
    if (demoMode) {
      if (body) addMessageBubble(document.getElementById('messageList'), body, 'Vous Â· DÃ©monstration', true);
  ...
  dashboard.addEventListener('click', async event => {
    ...
    if (!confirm('Supprimer dÃ©finitivement ce document?')) return;
    button.disabled = true;
    const { error: storageError } = await sb.storage.from('client-documents').remove([button.dataset.storagePath]);
    if (storageError) { button.disabled = false; return alert('La suppression a Ã©chouÃ©.'); }
    const { error: recordError } = await sb.from('documents').delete().eq('id', button.dataset.documentId);
    if (recordError) return alert('Le fichier a Ã©tÃ© supprimÃ©, mais le dossier doit Ãªtre actualisÃ©.');
    const { data: { user } } = await sb.auth.getUser();
